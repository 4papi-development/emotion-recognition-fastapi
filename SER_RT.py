...
# predict on pre-recorded audio
def predictAudioAndGet(filename: str):
    x_predictAudio = []
    featurePredictAudio = extract_feature(filename, mfcc=True, chroma=True, mel=True) #extract features of recorded audio
    x_predictAudio.append(featurePredictAudio)
    y_predictAudio = model.predict(np.array(x_predictAudio))
    print("Emotion Predicted: {}".format(y_predictAudio))
    return y_predictAudio[0]
